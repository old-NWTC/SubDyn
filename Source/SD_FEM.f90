...
!**********************************************************************************************************************************
! File last committed: $Date: 2016-06-24 12:16:38 -0600 (Fri, 24 Jun 2016) $
! (File) Revision #: $Rev: 372 $
! URL: $HeadURL: https://wind-dev2.nrel.gov/svn/SubDyn/branches/v1.03.00a-rrd/Source/SD_FEM.f90 $
!**********************************************************************************************************************************
MODULE SD_FEM
  USE NWTC_Library
  USE SubDyn_Types
...
!BJJ: TODO: for efficiency, this if check should be OUTSIDE the DO loop. 
   ...
   DO I=1, p%NReact  !Do this for all Constraint Nodes
     ...
     CALL STPTTR('U',6,Init%SSIK(I,:),SSIK,6,info)
     CALL STPTTR('U',6,Init%SSIM(I,:),SSIM,6,info)
     DO J=1,6
        SSIK(J,J)=SSIK(J,J)/2
        SSIM(J,J)=SSIM(J,J)/2
     ENDDO   
